...
public class EmployeeLinks : IEmployeeLinks
{
    ...
    {
        var employeesDtos = employeesDto.ToList();
        
        var shapedEmployees = ShapeData(employeesDtos, fields);
        return ShouldGenerateLinks(httpContext) 
            ? ReturnLinkedEmployees(employeesDtos, fields, companyId, httpContext, shapedEmployees)
            : ReturnShapedEmployees(shapedEmployees);
    }

    private List<Entity> ShapeData(IEnumerable<EmployeeDto> employeesDtos, string fields) =>
        _dataShaper.ShapeData(employeesDtos, fields).Select(e => e.Entity).ToList();
    ...
    private LinkResponse ReturnLinkedEmployees(IEnumerable<EmployeeDto> employeesDtos, string fields, Guid companyId,
    ...
    private List<Link> CreateLinksForEmployee(HttpContext httpContext, Guid companyId, Guid id, string fields = "")
    {
        ...
        {
            new (_linkGenerator.GetUriByAction(httpContext, "GetEmployeeForCompany", "Employees",
                values: new { companyId, id, fields})!,
            "self",
            "GET"),
                
            new (_linkGenerator.GetUriByAction(httpContext, "DeleteEmployeeForCompany", "Employees",
            values: new { companyId, id})!,
            "delete_employee",
            "DELETE"),
                
            new (_linkGenerator.GetUriByAction(httpContext, "UpdateEmployeeForCompany", "Employees",
            values: new { companyId, id})!,
            "update_employee",
            "UPDATE"),
                
            new (_linkGenerator.GetUriByAction(httpContext, "PartiallyUpdateEmployeeForCompany", "Employees",
            values: new { companyId, id})!,
